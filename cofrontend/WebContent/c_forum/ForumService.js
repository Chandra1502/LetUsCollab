...
app.service('ForumService',['$http','$q','$rootScope',function($http,$q,$rootScope){
	...
	var BASE_URL='http://localhost:8092/cobackend'
		return{
...
createForum:function(forum){
	...
	.then(
			...
			function(errResponse){
				console.error('Error while creating Forum in froumService.js');
				return $q.reject(errResponse);
			}				
);
},
...
updateForum:function(forum,id){
	...
	.then(
			...
			function(errResponse){
				console.error('Error while updating Forum in froumService.js');
				return $q.reject(errResponse);
			}				
);
},
...
deleteForum:function(id){
	...
	.then(
			...
			function(errResponse){
				console.error('Error while deleting Forum in froumService.js');
				return $q.reject(errResponse);
			}				
);
},
...
getForum: function(id){
	...
	.then(                                        //success handler,failure handler
			...
			function(errResponse){
				console.error('Error while getting Forum  in froumService.js');
				return $q.reject(errResponse);
			}				
);
},
